...
function fortune(string, length){
  ...
  this.next_transition = Math.floor(random(0, max_off_time));
    ...
    textSize(32);
    textFont("Harry P");
    text(this.string, pos[0] + width/2, pos[1] + 150);
...
function setup() {
  ...
  myCanvas.parent('ball');
	wordAng = 0;
	wordMag = 10;
	words.push(new fortune('IQP', 100));
	words.push(new fortune('Classes', 90));
	words.push(new fortune('Training', 80));
	words.push(new fortune('Display', 80));
	words.push(new fortune('Guide', 80));
}
